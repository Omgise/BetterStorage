...
public class PacketPresentOpen extends AbstractPacket<PacketPresentOpen> {
    ...
    public void handle(EntityPlayer player) {
        ...
        if (present != null) for (int side = 0; side < 6; side++) for (int i = 0; i < (destroy ? 2 : 20); i++)
            if (destroy) effRender.addBlockDestroyEffects(x, y, z, present.getBlockType(), present.getBlockMetadata());
