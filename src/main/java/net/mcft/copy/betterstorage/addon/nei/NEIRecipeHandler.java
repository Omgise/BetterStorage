...
public class NEIRecipeHandler extends TemplateRecipeHandler {
    ...
    public void drawBackground(int recipe) {
        ...
        mc.getTextureManager()
            .bindTexture(Resources.containerCraftingStation);
    ...
    public void loadCraftingRecipes(String outputId, Object... results) {
        if (outputId.equals(getOverlayIdentifier())) arecipes.addAll(getRecipes());
        else super.loadCraftingRecipes(outputId, results);
    }

    @Override
    public void loadCraftingRecipes(ItemStack result) {
        for (CachedStationRecipe recipe : getRecipes()) if (recipe.outputs(result)) arecipes.add(recipe);
    }

    @Override
    public void loadUsageRecipes(ItemStack ingredient) {
        for (CachedStationRecipe recipe : getRecipes()) if (recipe.uses(ingredient)) arecipes.add(recipe);
    }

    @Override
    public void onUpdate() {
        if (!NEIClientUtils.shiftKey() && ((++cycleticks % 20) == 0))
            for (CachedRecipe recipe : arecipes) ((CachedStationRecipe) recipe).cycle();
    }
    ...
    public class CachedStationRecipe extends CachedRecipe {
        ...
        public CachedStationRecipe(IStationRecipe recipe) {
            ...
            if (possibleInputs == null) {
                ...
                for (IRecipeInput[] inputArray : sampleInput)
                    for (IRecipeInput input : inputArray) if (input != null) possibleInputs.add(input);
            }
            ...
            if (possibleOutputs == null) {
                ...
                for (IRecipeInput[] inputArray : sampleInput) {
                    ...
                    for (int i = 0; i < inputArray.length; i++)
                        if (inputArray[i] != null) stackArray[i] = inputArray[i].getPossibleMatches()
                            .get(0);
                    for (ItemStack stack : recipe.checkMatch(stackArray, new RecipeBounds(stackArray))
                        .getOutput()) if (stack != null) possibleOutputs.add(stack);
                }
            }
        ...
        public void cycle() {
            ...
            if (crafting == null) throw new Error(
                "Recipe " + recipe.getClass()
                    .getSimpleName() + " didn't match sample input.");
            ...
            for (int x = 0; x < 3; x++)
                for (int y = 0; y < 3; y++) if (((x + y * 3) < outputArray.length) && (outputArray[x + y * 3] != null))
        ...
        public List<PositionedStack> getIngredients() {
            return getCycledIngredients(cycleticks / 20, ingridients);
        }
        ...
        public boolean outputs(ItemStack result) {
            for (ItemStack output : possibleOutputs)
                if (NEIServerUtils.areStacksSameTypeCrafting(output, result)) return true;
            return false;
        }

        public boolean uses(ItemStack ingredient) {
            for (IRecipeInput input : possibleInputs) if (input.matches(ingredient)) return true;
            return false;
        }
