...
public class CratePileCollection {
    ...
    public CratePileData getCratePile(int id) {
        ...
        if (!pileDataMap.containsKey(id)) {
            ...
            if (pileData == null) pileData = new CratePileData(this, id, 0);
    ...
    public void removeCratePile(CratePileData pileData) {
        ...
        if (pileDataMap.size() <= 0) collectionMap.remove(dimension);
    }
    ...
    public void save(CratePileData pileData) {
        try {
            ...
            if (file.exists() && !file.delete()) throw new Exception(file + " could not be deleted.");
    ...
    private File getSaveDirectory() {
        File saveFolder = world.getSaveHandler()
            .getWorldDirectory();
    ...
    public void onTick() {
        for (CratePileData data : pileDataMap.values()) data.blockView.onUpdate();
    }
