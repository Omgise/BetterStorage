...
public class InventoryStacks extends InventoryBetterStorage {
    ...
    public InventoryStacks(String name, ItemStack[]... allContents) {
        ...
        for (ItemStack[] contents : allContents) totalSize += contents.length;
    }
    ...
    private ItemStack[] getContentsAndSlot(int slot) {
        if (slot < 0 || slot >= totalSize) throw new IndexOutOfBoundsException("slot");
        tempSlot = slot;
        for (int i = 0;; i++) {
    ...
    public boolean equals(Object obj) {
        ...
        if ((getInventoryName() != inv.getInventoryName()) || (allContents.length != inv.allContents.length))
            return false;
        for (int i = 0; i < allContents.length; i++) if (allContents[i] != inv.allContents[i]) return false;
        return true;
    }
