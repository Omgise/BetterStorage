...
public class ItemPresentBook extends ItemEditableBook {
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        if (!world.isRemote || !ChristmasEventHandler.isBeforeChristmas()
            || (StackUtils.get(stack, 9001, "year") != ChristmasEventHandler.getYear())) return stack;
        ...
        NBTTagList pages = NbtUtils.createList(
            ...
            "  " + darkAqua
                ...
                + "Dear "
                + darkRed
                + StackUtils.get(stack, "[undefined]", "name")
                + black
                + ",\n"
                + "it's only "
                + days
            ...
            darkRed + " [ "
                ...
                + "Name tags"
                + black
                + " with the recipient's name will ensure only they can open their presents.\n\n"
                + "You can remove name tags in a "
                + darkRed
                + i
                + "crafting station"
                + black
    ...
    public static void showBookScreen(EntityPlayer player, NBTTagList pages) {
        ...
        Minecraft.getMinecraft()
            .displayGuiScreen(new GuiScreenBook(player, stack, false));
    }
