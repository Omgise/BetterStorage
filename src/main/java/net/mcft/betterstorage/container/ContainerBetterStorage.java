package net.mcft.betterstorage.container;

import net.mcft.betterstorage.client.gui.GuiBetterStorage;
...
public class ContainerBetterStorage extends Container {
    ...
    protected void setupInventoryContainer() {
        for (int y = 0; y < getRows(); y++) for (int x = 0; x < getColumns(); x++)
            addSlotToContainer(new SlotBetterStorage(this, inventory, x + y * getColumns(), 8 + x * 18, 18 + y * 18));
    }

    protected void setupInventoryPlayer() {
        for (int y = 0; y < 3; y++) for (int x = 0; x < 9; x++) addSlotToContainer(
            new SlotBetterStorage(
                this,
                player.inventory,
                9 + x + y * 9,
                ...
                getHeight() - 83 + y * 18));
        setHotbarStart();
        for (int x = 0; x < 9; x++) addSlotToContainer(
            new SlotBetterStorage(this, player.inventory, x, 8 + x * 18 + (getColumns() - 9) * 9, getHeight() - 25));
    }
    ...
    protected boolean mergeItemStack(ItemStack stack, int start, int end, boolean backwards) {
        ...
        if (stack.isStackable()) {
            for (int i = 0; i < (end - start); i++) {
                ...
                if (StackUtils.matches(stack, slotStack) && (slotStack.stackSize < maxStackSize)) {
                    ...
                    if (slot.isItemValid(testStack) && slot.inventory.isItemValidForSlot(slot.slotNumber, testStack)) {
    ...
    public ItemStack slotClick(int slotId, int button, int special, EntityPlayer player) {
        Slot slot = null;
        if ((slotId >= 0) && (slotId < inventorySlots.size())) slot = (Slot) inventorySlots.get(slotId);
        if (slot != null) {
            if (special == 0) {
                if ((button == 0) || (button == 1)) {
                    ...
                    if ((slotStack != null) && (holding != null)
                        ...
                        if (amount > 0) {
                            if ((holding.stackSize -= amount) <= 0) player.inventory.setItemStack(null);
                ...
                if (!slot2.canTakeStack(player) || ((stack != null) && !slot2.isItemValid(stack))) return null;
            }
        }
    ...
    public void sendUpdate(int id, int value) {
        for (Object c : crafters) ((ICrafting) c).sendProgressBarUpdate(this, id, value);
    }
    ...
    public void sendUpdateIfChanged(int id, int value, int previousValue) {
        if (value != previousValue) sendUpdate(id, value);
    }
