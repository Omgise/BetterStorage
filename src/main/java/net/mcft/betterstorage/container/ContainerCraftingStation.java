...
import net.mcft.betterstorage.inventory.InventoryCraftingStation;
...
public class ContainerCraftingStation extends ContainerBetterStorage {
    ...
    protected void setupInventoryContainer() {
        inv = ((inventory instanceof InventoryCraftingStation) ? (InventoryCraftingStation) inventory
            : ((InventoryCraftingStation) ((InventoryTileEntity) inventory).inventory));

        // Crafting
        for (int y = 0; y < 3; y++) for (int x = 0; x < 3; x++)
            addSlotToContainer(new SlotBetterStorage(this, inventory, x + y * 3, 17 + x * 18, 17 + y * 18));
        // Output
        for (int y = 0; y < 3; y++) for (int x = 0; x < 3; x++)
            addSlotToContainer(new SlotStationOutput(this, inventory, 9 + x + y * 3, 107 + x * 18, 17 + y * 18));
        // Inventory
        for (int y = 0; y < getRows(); y++) for (int x = 0; x < getColumns(); x++) addSlotToContainer(
            new SlotBetterStorage(this, inventory, 18 + x + y * getColumns(), 8 + x * 18, 76 + y * 18));
    }
    ...
    public void detectAndSendChanges() {
        ...
        int progress = ((inv.currentCrafting != null) ? Math.min(inv.progress, inv.currentCrafting.getCraftingTime())
            : 0);
    ...
    public ItemStack slotClick(int slotId, int button, int special, EntityPlayer player) {
        if (!inv.outputIsReal && (inv.currentCrafting != null)
            && (slotId >= 9)
            && (slotId < 18)
            && (inv.output[slotId - 9] != null)
            && inv.canTake(player)
            && (special != 3)) {
            ...
            if (special == 1) {
                ...
                if ((holding == null) || (StackUtils.matches(holding, craftingStack)
                    && (holding.stackSize <= holding.getMaxStackSize() - amount))) inv.craft(player);
