...
import net.mcft.betterstorage.item.IDyeableItem;
import net.mcft.betterstorage.item.ItemBackpack;
import net.mcft.betterstorage.item.ItemBucketSlime;
import net.mcft.betterstorage.item.cardboard.ICardboardItem;
import net.mcft.betterstorage.item.cardboard.ItemCardboardSheet;
...
import net.mcft.betterstorage.misc.handlers.BackpackHandler;
import net.mcft.betterstorage.misc.handlers.CraftingHandler;
import net.mcft.betterstorage.tile.crate.CratePileCollection;
import net.mcft.betterstorage.tile.entity.TileEntityLockableDoor;
import net.mcft.betterstorage.tile.stand.VanillaArmorStandEquipHandler;
...
public class CommonProxy {
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (leftClick || rightClick) {
            ...
            if (hasAttachments != null) {
                EnumAttachmentInteraction interactionType = ((event.action == Action.LEFT_CLICK_BLOCK)
                    ? EnumAttachmentInteraction.attack
                    : EnumAttachmentInteraction.use);
                if (hasAttachments.getAttachments()
                    .interact(WorldUtils.rayTrace(player, 1.0F), player, interactionType)) {
        ...
        if (leftClick && (holding != null)
            ...
            && !ItemCardboardSheet.isEffective(holding)) event.useItem = Result.DENY;
    ...
    protected AxisAlignedBB getIronDoorHightlightBox(EntityPlayer player, World world, int x, int y, int z, Vec3 hitVec,
        Block block) {
        ...
        switch (rotation) {
            case 0:
                if (!isOpen) box = AxisAlignedBB.getBoundingBox(
                    x - 0.005 / 16F,
                    y + 14.5 / 16F,
                    z + 10 / 16F,
                    x + 3.005 / 16F,
                    y + 20.5 / 16F,
                    z + 15 / 16F);
                else box = AxisAlignedBB.getBoundingBox(
                    x + 10 / 16F,
                    y + 14.5 / 16F,
                    z - 0.005 / 16F,
                    x + 15 / 16F,
    ...
    public void onBreakSpeed(BreakSpeed event) {
        ...
        if ((holding != null) && (holding.getItem() instanceof ICardboardItem)
            && !ItemCardboardSheet.isEffective(holding)) event.newSpeed = -1;
    }
    ...
    public void onEntityInteract(EntityInteractEvent event) {
        ...
        if ((BetterStorageItems.slimeBucket != null) && (target instanceof EntityLiving)
            && (holding != null)
            && (holding.getItem() == Items.bucket)) {
            ItemBucketSlime.pickUpSlime(player, (EntityLiving) target);
            if (player.getCurrentEquippedItem()
                .getItem() instanceof ItemBucketSlime) preventSlimeBucketUse = true;
        }
    ...
    public void onWorldTick(WorldTickEvent event) {
        if (event.side == Side.SERVER) CratePileCollection.getCollection(event.world)
            .onTick();
    }
