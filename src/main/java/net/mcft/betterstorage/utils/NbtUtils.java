...
public final class NbtUtils {
    ...
    public static NBTTagList createList(Object... values) {
        ...
        for (Object value : values) list.appendTag((value instanceof NBTBase) ? (NBTBase) value : createTag(value));
        return list;
    }
    ...
    public static void readItems(ItemStack[] contents, NBTTagList items) {
        ...
        for (int i = 0; i < items.tagCount(); i++) {
            ...
            if (slot >= 0 && slot < contents.length) contents[slot] = ItemStack.loadItemStackFromNBT(item);
        }
    }

    public static void readItems(List<ItemStack> list, NBTTagList items) {
        for (int i = 0; i < items.tagCount(); i++) list.add(ItemStack.loadItemStackFromNBT(items.getCompoundTagAt(i)));
    }
