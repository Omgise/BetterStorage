...
public final class WorldUtils {
    ...
    public static AxisAlignedBB getAABB(TileEntity entity, double minX, double minY, double minZ, double maxX,
    ...
    public static boolean isTileEntityUsableByPlayer(TileEntity entity, EntityPlayer player) {
        return (entity.getWorldObj()
            .getTileEntity(entity.xCoord, entity.yCoord, entity.zCoord) == entity
    ...
    public static int syncPlayersUsing(TileEntity te, int playersUsing, IInventory playerInventory) {
        if (!te.getWorldObj().isRemote && (playersUsing != 0)) {
            playersUsing = 0;
            List<EntityPlayer> players = te.getWorldObj()
                .getEntitiesWithinAABB(EntityPlayer.class, getAABB(te, 5));
    ...
    /**
     * This will perform a {@link World#notifyBlockOfNeighborChange()} on every adjacent block including the block at
     * x|y|z.
     */
    ...
    public static MovingObjectPosition rayTrace(EntityPlayer player, float partialTicks) {
        ...
        double range = ((player.worldObj.isRemote) ? Minecraft.getMinecraft().playerController.getBlockReachDistance()
