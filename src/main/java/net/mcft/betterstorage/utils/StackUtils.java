package net.mcft.betterstorage.utils;
...
public final class StackUtils {
    ...
    public static boolean matches(Item item1, int damage1, NBTTagCompound data1, Item item2, int damage2,
    ...
    public static boolean matches(ItemStack stack1, ItemStack stack2, boolean matchNBT) {
        return ((stack1 == null) ? (stack2 == null)
            : ((stack2 != null) && matches(
                stack1.getItem(),
                StackUtils.getRealItemDamage(stack1),
                (matchNBT ? stack1.getTagCompound() : null),
                stack2.getItem(),
                StackUtils.getRealItemDamage(stack2),
                (matchNBT ? stack2.getTagCompound() : null))));
    }
    ...
    public static Map<Integer, StackEnchantment> getEnchantments(ItemStack stack) {
        ...
        NBTTagList list = ((stack.getItem() == Items.enchanted_book) ? Items.enchanted_book.func_92110_g(stack)
    ...
    public static boolean isEnchantmentCompatible(ItemStack stack, Collection<StackEnchantment> stackEnchants,
        StackEnchantment newEnchant) {
        if (!newEnchant.ench.canApply(stack)) return false;
        for (StackEnchantment stackEnch : stackEnchants) if ((newEnchant.ench == stackEnch.ench)
    ...
    public static int stackItems(ItemStack[] contents, List<ItemStack> items) {
        int numStacks = 0;
        outerLoop: for (int i = 0; i < contents.length; i++) {
            ItemStack contentStack = contents[i];
            if (contentStack == null) continue;
            numStacks++;
            for (ItemStack itemsStack : items) if (StackUtils.matches(contentStack, itemsStack)) {
