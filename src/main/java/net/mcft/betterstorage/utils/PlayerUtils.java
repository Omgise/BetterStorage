...
public final class PlayerUtils {
    ...
    public static void openGui(EntityPlayer pl, String name, int columns, int rows, String title, Container container) {
        ...
        BetterStorage.networkChannel
            .sendTo(new PacketOpenGui(player.currentWindowId, name, columns, rows, title), player);
    ...
    private static GuiScreen createGuiFromName(EntityPlayer player, String name, int columns, int rows, String title) {
        ...
        if (name.equals(Constants.containerCrate)) return new GuiCrate(player, rows, title, localized);
        ...
        else if (name.equals(Constants.containerCardboardBox)) return new GuiBetterStorage(
            player,
            columns,
            rows,
