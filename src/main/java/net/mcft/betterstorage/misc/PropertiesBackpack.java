...
public class PropertiesBackpack implements IExtendedEntityProperties {
    ...
    public void saveNBTData(NBTTagCompound compound) {
        ...
        if (backpack != null) backpackCompound.setTag("Stack", backpack.writeToNBT(new NBTTagCompound()));
        ...
        if (!entity.worldObj.isRemote) {
            ...
            if (isOpen != (prevPlayersUsing > 0)) {
                BetterStorage.networkChannel
                    .sendToAndAllTracking(new PacketBackpackIsOpen(entity.getEntityId(), isOpen), entity);
                prevPlayersUsing = playersUsing;
            }
            if (!ItemStack.areItemStacksEqual(backpack, prevBackpack)) {
                BetterStorage.networkChannel
                    .sendToAndAllTracking(new PacketBackpackStack(entity.getEntityId(), backpack), entity);
    ...
    public void sendDataToPlayer(EntityLivingBase entity, EntityPlayer player) {
        ...
        if (backpack != null)
            BetterStorage.networkChannel.sendTo(new PacketBackpackStack(entity.getEntityId(), backpack), player);
    }
