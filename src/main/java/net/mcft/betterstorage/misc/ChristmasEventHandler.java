...
public class ChristmasEventHandler {
        ...
        if (isPresentTime() && !properties.gotPresent) {
            ...
            for (int i = 0; i < inv.getSizeInventory(); i++) {
                ...
                if ((stack != null) && (stack.getItem() == BetterStorageItems.presentBook)
                    ...
                    && event.player.getUniqueID()
                        .toString()
                        .equals(StackUtils.get(stack, null, "uuid"))) {
                    ...
                    if (event.player.getCommandSenderName()
                        .equalsIgnoreCase("xXxCJxXx")) {
                        StackUtils.set(present, (byte) 1, TileEntityPresent.TAG_SKOJANZA_MODE);
                        StackUtils.set(
                            present,
                            NbtUtils.createList("Just for you!", "skojanzaMode = true"),
                            "display",
                            "Lore");
                    }
    ...
    private static void getItemsFor2014(List<ItemStack> items, EntityPlayer player) {
        ...
        ItemStack sword = new ItemStack(
            (BetterStorageItems.cardboardSword != null) ? BetterStorageItems.cardboardSword : Items.wooden_sword);
        ...
        if (BetterStorageItems.cardboardSword != null) {
            ...
            StackUtils.set(
                sword,
                NbtUtils.createList("True strength is not in power,", "but in overcoming challenges."),
                "display",
                "Lore");
        } else {
            ...
            StackUtils.set(
                sword,
                NbtUtils.createList("Not made of actual cardboard,", "because some douche disabled it."),
                "display",
                "Lore");
        }
        items.add(sword);
        ...
        ItemStack chestplate = new ItemStack(
            (BetterStorageItems.cardboardChestplate != null) ? BetterStorageItems.cardboardChestplate
        ...
        if (BetterStorageItems.drinkingHelmet != null) {
            ...
            StackUtils.set(
                weaknessPotion,
                NbtUtils.createList(NbtUtils.createCompound("Id", 18, "Amplifier", 2, "Duration", 2000)),
                "CustomPotionEffects");
        ...
        if (BetterStorageItems.slimeBucket != null) {
            ...
            StackUtils.set(
                slime,
                NbtUtils.createList(
                    new PotionEffect(Potion.regeneration.id, 6000, 0)
                    ...
                    new PotionEffect(Potion.fireResistance.id, 6000, 2)
                        .writeCustomPotionEffectToNBT(new NBTTagCompound()),
                    new PotionEffect(Potion.waterBreathing.id, 6000, 3)
                        .writeCustomPotionEffectToNBT(new NBTTagCompound()),
                    new PotionEffect(Potion.nightVision.id, 6000, 3)
                        .writeCustomPotionEffectToNBT(new NBTTagCompound())),
                "Effects");
            StackUtils.set(
                slime,
                NbtUtils.createList("Sneak and use to consume.", "Don't accidentially release it!"),
                "display",
                "Lore");
            items.add(slime);
        } else vouchers = createVoucher(items, "Slime in a Bucket");

        if (BetterStorageTiles.craftingStation != null) items.add(new ItemStack(BetterStorageTiles.craftingStation));
    ...
    private static boolean createVoucher(List<ItemStack> items, String itemName) {
        ...
        StackUtils.set(
            voucher,
            NbtUtils.createList("Item: " + itemName, "Redeem at server owner / modpack author."),
            "display",
            "Lore");
        items.add(voucher);
        return true;
