...
import net.mcft.betterstorage.api.BetterStorageBackpack;
...
import net.mcft.betterstorage.inventory.InventoryStacks;
import net.mcft.betterstorage.item.ItemBackpack;
import net.mcft.betterstorage.item.ItemEnderBackpack;
import net.mcft.betterstorage.misc.EquipmentSlot;
import net.mcft.betterstorage.misc.PropertiesBackpack;
import net.mcft.betterstorage.network.packet.PacketSyncSetting;
import net.mcft.betterstorage.tile.TileEnderBackpack;
import net.mcft.betterstorage.utils.DirectionUtils;
import net.mcft.betterstorage.utils.EntityUtils;
import net.mcft.betterstorage.utils.NbtUtils;
import net.mcft.betterstorage.utils.RandomUtils;
import net.mcft.betterstorage.utils.ReflectionUtils;
...
public class BackpackHandler {
    ...
    private static final WeightedRandomChestContent[] randomBackpackItems = new WeightedRandomChestContent[] {
        ...
        makeWeightedContent(Items.stick, 8, 20, 100), makeWeightedContent(Blocks.planks, 2, 10, 100),
        makeWeightedContent(Blocks.log, 1, 8, 40), makeWeightedContent(Blocks.cobblestone, 6, 16, 80),

        makeWeightedContent(Items.wooden_pickaxe, 50, 35), makeWeightedContent(Items.wooden_pickaxe, 20, 10),
        makeWeightedContent(Items.stone_pickaxe, 120, 10), makeWeightedContent(Items.stone_pickaxe, 80, 5),
        makeWeightedContent(Items.iron_pickaxe, 220, 2),

        makeWeightedContent(Items.wooden_sword, 40, 30), makeWeightedContent(Items.stone_sword, 60, 5),

        makeWeightedContent(Items.bow, 200, 1, 1, 10), makeWeightedContent(Items.bow, 50, 1, 1, 3),
        makeWeightedContent(Items.fishing_rod, 20, 1, 1, 4), makeWeightedContent(Items.compass, 1, 1, 6),
        makeWeightedContent(Items.clock, 1, 1, 5),

        makeWeightedContent(Blocks.torch, 6, 24, 30), makeWeightedContent(Items.arrow, 2, 12, 10),
        makeWeightedContent(Items.rotten_flesh, 0, 3, 6, 15), makeWeightedContent(Items.bone, 2, 5, 20),
        makeWeightedContent(Items.string, 3, 10, 15),

        makeWeightedContent(Items.apple, 2, 5, 15), makeWeightedContent(Items.bread, 2, 4, 10),
        makeWeightedContent(Items.wheat, 3, 6, 10), makeWeightedContent(Items.carrot, 1, 2, 8),
        makeWeightedContent(Items.potato, 1, 2, 5), makeWeightedContent(Items.fish, 1, 4, 5),
        makeWeightedContent(Items.cooked_fished, 1, 2, 4),

        makeWeightedContent(Items.coal, 3, 9, 20), makeWeightedContent(Items.coal, 20, 32, 5),
        makeWeightedContent(Blocks.iron_ore, 2, 5, 15), makeWeightedContent(Blocks.iron_ore, 10, 20, 2),
        makeWeightedContent(Blocks.gold_ore, 2, 7, 8), makeWeightedContent(Items.diamond, 1, 2, 1),
    ...
    public void onEntityConstructing(EntityConstructing event) {
        ...
        if (event.entity instanceof EntityLivingBase) ItemBackpack.initBackpackData((EntityLivingBase) event.entity);
    }
    ...
    public void onEntityInteract(EntityInteractEvent event) {
        ...
        if (event.entity.worldObj.isRemote || !(event.entity instanceof EntityPlayerMP)
            ...
            || ((event.target instanceof EntityPlayer)
                && !BetterStorage.globalConfig.getBoolean(GlobalConfig.enableBackpackInteraction)))
            return;

        EntityPlayerMP player = (EntityPlayerMP) event.entity;
        EntityLivingBase target = (EntityLivingBase) event.target;
        if (ItemBackpack.openBackpack(player, target)) player.swingItem();
    ...
    public void onSpecialSpawn(SpecialSpawn event) {
        ...
        for (BetterStorageBackpack.BackpackSpawnEntry entry : BetterStorageBackpack.spawnWithBackpack) {
            if (!entity.getClass()
                .equals(entry.entityClass)) continue;
        ...
        } else if (BetterStorageTiles.backpack != null) ItemBackpack.getBackpackData(entity).spawnsWithBackpack = true;
    ...
    public void onPlayerStartTracking(PlayerEvent.StartTracking event) {
        ...
        if (event.target instanceof EntityLivingBase) ItemBackpack.getBackpackData((EntityLivingBase) event.target)
    ...
    public void onPlayerChangedDimensionEvent(PlayerChangedDimensionEvent event) {
        ...
        ItemBackpack.getBackpackData(event.player)
            .sendDataToPlayer(event.player, event.player);
    }
    ...
    public void onLivingUpdate(LivingUpdateEvent event) {
            ...
            if (backpackData.spawnsWithBackpack) {
                ...
                if (contents != null) {
                    ...
                    WeightedRandomChestContent
                        .generateChestContents(RandomUtils.random, randomBackpackItems, inventory, 20);
                    // With a chance of 10%, add some random dungeon loot.
                    if (RandomUtils.getDouble() < 0.1) {
                        ChestGenHooks info = ChestGenHooks.getInfo(ChestGenHooks.DUNGEON_CHEST);
                        WeightedRandomChestContent
                            .generateChestContents(RandomUtils.random, info.getItems(RandomUtils.random), inventory, 5);
                    }
        ...
        ItemBackpack.getBackpackData(entity)
            .update(entity);
    ...
    public void onLivingDeath(LivingDeathEvent event) {
        ...
        boolean keepInventory = entity.worldObj.getGameRules()
        ...
        if ((player != null) && keepInventory) {
            ...
            if (BetterStorage.globalConfig.getBoolean(GlobalConfig.dropBackpackOnDeath)) {
                ...
                if (backpackData.backpack != null) WorldUtils.dropStackFromEntity(entity, backpack, 4.0F);
    ...
    private static class BlockCoordinate {
        ...
        public BlockCoordinate(double ex, double ey, double ez, int x, int y, int z) {
            ...
            this.z = (int) ez + z;
            distance = Math
                .sqrt(Math.pow(this.x + 0.5 - ex, 2) + Math.pow(this.y + 0.5 - ey, 2) + Math.pow(this.z + 0.5 - ez, 2));
        }
    }
    ...
    public void onEntityJoinWorldEvent(EntityJoinWorldEvent event) {
        ...
        ItemStack stack = entity.getDataWatcher()
            .getWatchableObjectItemStack(10);
        if ((stack == null) || !(stack.getItem() instanceof ItemEnderBackpack)) return;
        event.setCanceled(true);
        for (int i = 0; i < 64; i++) if (TileEnderBackpack
            .teleportRandomly(entity.worldObj, entity.posX, entity.posY, entity.posZ, (i > 48), stack)) break;
    ...
    public void onPlayerRespawn(PlayerRespawnEvent event) {
        ...
        if (persistent.hasNoTags()) entityData.removeTag(EntityPlayer.PERSISTED_NBT_TAG);
