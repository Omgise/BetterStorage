...
import net.mcft.betterstorage.api.crafting.ShapedStationRecipe;
...
public class MTShapedStationRecipe extends ShapedStationRecipe {
    ...
    public MTShapedStationRecipe(IIngredient[][] ingredients, ItemStack[] output, int experience, int craftingTime,
        boolean mirrored) {
    ...
    private static IRecipeInput[] toRecipeInputs(IIngredient[][] ingredients) {
        ...
        for (int x = 0; x < width; x++) for (int y = 0; y < height; y++)
            if (ingredients[y][x] != null) inputs[x + y * width] = new RecipeInputIngredient(ingredients[y][x]);
        return inputs;
    }
