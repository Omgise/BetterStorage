...
import net.mcft.betterstorage.api.crafting.RecipeInputBase;
...
public class RecipeInputIngredient extends RecipeInputBase {
    ...
    public void craft(ItemStack input, ContainerInfo containerInfo) {
        ...
        ItemStack transformed = MTHelper.toStack(
            ingredient.applyTransform(new MCItemStack(input), ((player != null) ? new MCPlayer(player) : null)));
        if (transformed != input) {
            input.stackSize = 0;
            if (transformed.stackSize > 0) containerInfo.set(transformed, false);
        }
    }
