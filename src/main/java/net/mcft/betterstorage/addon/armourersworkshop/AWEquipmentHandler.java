...
public class AWEquipmentHandler extends ArmorStandEquipHandler {
    ...
    public AWEquipmentHandler(ISkinType type, int priority) {
        super(
            "ArmourersWorkshop " + type.getRegistryName(),
            EnumArmorStandRegion.values()[3 - type.getVanillaArmourSlotId()],
            priority);
        this.type = type;
    }
    ...
    public boolean isValidItem(EntityPlayer player, ItemStack item) {
        return AWAddon.dataHandler.isValidEquipmentSkin(item) && AWAddon.dataHandler.getSkinPointerFromStack(item)
            .getSkinType() == type;
    }
