package net.mcft.betterstorage.entity;
...
public class EntityFrienderman extends EntityEnderman {
    ...
    public boolean attackEntityFrom(DamageSource source, float damage) {
        ...
        if (entityToAttack != null) {
            for (int i = 0; i < 40 - getMaxHealth() / 2; i++) if (teleportRandomly()) break;
            entityToAttack = null;
        }
        return success;
    }
    ...
    protected void despawnEntity() {
        ...
        else if ((distance > 64) && (entityAge > 20 * 60 * 5) && RandomUtils.getBoolean(0.001)) setDead();
    }
    ...
    public void onLivingUpdate() {
        ...
        if (ruleChanged) rules.setOrCreateGameRule("mobGriefing", ruleBefore);
        ...
        if (!hasEnderChest && (func_146080_bZ() == Blocks.ender_chest)) {
            ...
            WorldUtils.get(worldObj, x, y, z, TileEntityBackpack.class).stack = new ItemStack(
                BetterStorageItems.itemEnderBackpack);
            double px = x + 0.5;
            double py = y + 0.5;
            double pz = z + 0.5;
            BetterStorage.networkChannel
                .sendToAllAround(new PacketBackpackTeleport(px, py, pz, x, y, z), worldObj, px, py, pz, 256);
