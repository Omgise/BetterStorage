package net.mcft.betterstorage.api.crafting;
...
public class ShapedStationRecipe implements IStationRecipe {
    ...
    public ShapedStationRecipe(ItemStack[] output, Object... input) {
        if (input.length <= 0) throw new IllegalArgumentException("input has no elements.");
        ...
        while ((inputIndex < input.length) && (input[inputIndex] instanceof String)) {
            ...
            if (line.isEmpty()) throw new IllegalArgumentException("Empty string isn't valid.");
            if (width <= 0) width = line.length();
            else if (width != line.length())
                throw new IllegalArgumentException("All strings must have the same length.");
            for (char chr : line.toCharArray()) inputMap.put(chr, null);
            height++;
        }
        if (height <= 0) throw new IllegalArgumentException("At least one string must be supplied.");

        if (inputIndex >= input.length) throw new IllegalArgumentException("At least one mapping must be supplied.");
        ...
        for (int x = 0; x < width; x++)
            for (int y = 0; y < height; y++) recipeInput[x + y * width] = inputMap.get(((String) input[y]).charAt(x));
    ...
    public StationCrafting checkMatch(ItemStack[] input, RecipeBounds bounds) {
        ...
        if ((recipeWidth != bounds.getWidth()) || (recipeHeight != bounds.getHeight())) return null;

        IRecipeInput[] requiredInput = new IRecipeInput[9];
        if (!checkMatch(input, bounds, requiredInput, false)
            && !(mirrored && checkMatch(input, bounds, requiredInput, true))) return null;
        ...
        for (int x = 0; x < recipeWidth; x++) {
            for (int y = 0; y < recipeHeight; y++) {
                ...
                if ((recipeStack == null) ? (inputStack != null)
                    : (!recipeStack.matches(inputStack) || (inputStack.stackSize < recipeStack.getAmount())))
                    return false;
            }
        }
        return true;
    ...
    public static int[] calculateRecipeBounds(ItemStack[] input) {
        ...
        for (int x = minX; x <= maxX; x++) for (int y = minY; y <= maxY; y++) if (input[x + y * 3] != null) {
            minX = x;
            break;
        }

        for (int x = maxX; x >= minX; x--) for (int y = minY; y <= maxY; y++) if (input[x + y * 3] != null) {
            maxX = x;
            break;
        }

        for (int y = minY; y <= maxY; y++) for (int x = minX; x <= maxX; x++) if (input[x + y * 3] != null) {
            minY = y;
            break;
        }

        for (int y = maxY; y >= minY; y--) for (int x = minX; x <= maxX; x++) if (input[x + y * 3] != null) {
            maxY = y;
            break;
        }
