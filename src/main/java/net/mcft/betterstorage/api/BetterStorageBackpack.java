package net.mcft.betterstorage.api;
...
public final class BetterStorageBackpack {
    ...
    public static ItemStack getBackpack(EntityLivingBase entity) {
        if (Loader.isModLoaded("betterstorage")) {
            try {
                if (getBackpack == null) {
                    Class itemBackpack = Class.forName("net.mcft.betterstorage.item.ItemBackpack");
                    getBackpack = itemBackpack.getMethod("getBackpack", EntityLivingBase.class);
                }
                return (ItemStack) getBackpack.invoke(null, entity);
            } catch (Exception e) {
                FMLLog
                    .warning("[betterstorage] Could not invoke item.net.mcft.betterstorage.ItemBackpack.getBackpack.");
            }
        }
        return null;
    }
