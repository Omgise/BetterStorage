...
public class ItemCardboardBox extends ItemBlock implements IContainerItem, IDyeableItem {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean advancedTooltips) {
        ...
        outerLoop: for (int i = 0; i < contents.length; i++) {
            ...
            for (DisplayNameStack itemsStack : items) if (itemsStack.matchAndAdd(contentStack)) continue outerLoop;
            items.add(new DisplayNameStack(contentStack));
        }

        Collections.sort(items);
        for (int i = 0; (i < items.size()) && (i < limit); i++) list.add(
            items.get(i)
                .toString());
        ...
        int count = 0;
        for (int i = limit; i < items.size(); i++) count += items.get(i).stackSize;
    ...
    public ItemStack[] getContainerItemContents(ItemStack container) {
        if (StackUtils.has(container, "Items")) return StackUtils.getStackContents(container, getRows());
        else return null;
    }
