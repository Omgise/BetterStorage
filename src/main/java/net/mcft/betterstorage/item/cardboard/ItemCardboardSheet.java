...
public class ItemCardboardSheet extends ItemBetterStorage {
    ...
    public static final ArmorMaterial armorMaterial = EnumHelper
        .addArmorMaterial("cardboard", 5, new int[] { 1, 2, 2, 1 }, 0);
    ...
    public static boolean damageItem(ItemStack stack, int damage, EntityLivingBase entity) {
        ...
        int numLevelTotal = 0;
        for (int enchLevel : enchants.values()) numLevelTotal += enchLevel;
        ...
        if (entity.worldObj.rand.nextDouble() >= changeForNoDamage) stack.damageItem(1, entity);
    ...
    public static boolean onBlockDestroyed(World world, Block block, int x, int y, int z, ItemStack stack,
        EntityLivingBase entity) {
        return ((block.getBlockHardness(world, x, y, z) > 0) ? ItemCardboardSheet.damageItem(stack, 1, entity) : true);
    }
