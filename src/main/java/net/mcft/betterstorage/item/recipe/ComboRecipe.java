...
import net.mcft.betterstorage.api.BetterStorageUtils;
...
public abstract class ComboRecipe implements IRecipe {
    ...
    public boolean matches(InventoryCrafting crafting, World world) {
        for (int x = 0; x <= 3 - width; ++x) for (int y = 0; y <= 3 - height; ++y) {
    ...
    private boolean checkMatch(InventoryCrafting crafting, int startX, int startY, boolean mirror) {
        ...
        for (int x = 0; x < 3; ++x) for (int y = 0; y < 3; ++y) {
            ...
            if ((craftingStack != null) || (recipeStack != null)) {
                ...
                if ((recipeStack == null) && (craftingStack != null)) shapelessItems.add(craftingStack);
                else if (!BetterStorageUtils.wildcardMatch(recipeStack, craftingStack)) return false;
            }
        }
        if ((shapelessItems.size() > 0) && !checkShapelessItems(crafting, shapelessItems)) return false;
        return true;
    }
