...
public class KeyRecipe extends ComboRecipe {
    ...
    public boolean checkShapelessItems(InventoryCrafting crafting, List<ItemStack> shapelessItems) {
        ...
        for (ItemStack stack : shapelessItems) {
            if ((stack.getItem() != BetterStorageItems.key) && (!DyeUtils.isDye(stack))) return false;
        }
        return true;
    }
    ...
    public ItemStack getCraftingResult(InventoryCrafting crafting) {
        ...
        if (key != null) ItemBetterStorage.setID(result, ItemBetterStorage.getID(key));
        // Apply color.
        if (dyes.size() > 0) {
    ...
    public static KeyRecipe createKeyRecipe(Object... recipe) {
        ...
        for (Object obj : recipe) {
            ...
            } else if (obj instanceof Character) lastChar = (Character) obj;
            else if (obj instanceof ItemStack) itemMap.put(lastChar, (ItemStack) obj);
            else if (obj instanceof Item) itemMap.put(lastChar, new ItemStack((Item) obj));
            else if (obj instanceof Block) itemMap.put(lastChar, new ItemStack((Block) obj));
        }
        ...
        for (int y = 0; y < height; y++) {
            ...
            for (int x = 0; x < width; x++) {
                ...
                if (x < recipeString.length()) {
                    ...
                    if (itemMap.containsKey(chr)) item = itemMap.get(chr);
                }
