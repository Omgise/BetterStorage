...
public class ItemBucketSlime extends ItemBetterStorage {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean advancedTooltips) {
        ...
        if ((name != null) || advancedTooltips) list.add(
            "Contains: " + ((name != null) ? ("\"" + name + "\"" + (advancedTooltips ? " (" + id + ")" : "")) : id));
        ...
        if ((effectList != null) && (handler != null)) {
            ...
            for (int i = 0; i < Math.min(effectList.tagCount(), max); i++) {
                ...
                if (effect.getAmplifier() > 0) str.append(" ")
                    .append(StatCollector.translateToLocal("potion.potency." + effect.getAmplifier()));
                str.append(" (")
                    .append(StringUtils.ticksToElapsedTime(duration))
                    .append(")");
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if ((entity != null) && (handler != null) && (entity instanceof EntityLiving)) {
            ...
            NBTTagList effectList = (NBTTagList) StackUtils.getTag(stack, "Effects");
            if (effectList != null) for (int i = 0; i < effectList.tagCount(); i++)
    ...
    public ItemStack onEaten(ItemStack stack, World world, EntityPlayer player) {
        ...
        if (handler != null) {
            player.getFoodStats()
                .addStats(handler.foodAmount(), handler.saturationAmount());
            NBTTagList effectList = (NBTTagList) StackUtils.getTag(stack, "Effects");
            if (effectList != null) {
    ...
    public static void pickUpSlime(EntityPlayer player, EntityLiving slime) {
        ...
        if (slime.isDead || (handler == null) || (handler.getSize(slime) != 1)) return;

        ItemStack stack = new ItemStack(BetterStorageItems.slimeBucket);

        String entityId = EntityList.getEntityString(slime);
        if (!entityId.equals("Slime")) StackUtils.set(stack, entityId, "Slime", "id");
        if (slime.hasCustomNameTag()) StackUtils.set(stack, slime.getCustomNameTag(), "Slime", "name");
        ...
        if (--player.getCurrentEquippedItem().stackSize <= 0) player.setCurrentItemOrArmor(EquipmentSlot.HELD, stack);
        else if (!player.inventory.addItemStackToInventory(stack)) player.dropPlayerItemWithRandomChoice(stack, true);
        ...
        registerHandler(new Handler("magmaCube", "LavaSlime") {
            ...
            public void onEaten(EntityPlayer player, boolean potionEffects) {
                ...
                player.addPotionEffect(
                    new PotionEffect(Potion.jump.id, (potionEffects ? 10 : 20) * 20, (potionEffects ? 2 : 3)));
            ...
            public void onEaten(EntityPlayer player, boolean potionEffects) {
                if (potionFluxId == -1) {
                    ...
                    for (Potion potion : Potion.potionTypes)
                        if (potion != null) if (potion.getName() == "potion.fluxtaint") {
                            potionFluxId = potion.id;
                            break;
                        }
                    // If not found, just use wither.
                    if (potionFluxId == -1) potionFluxId = Potion.wither.id;
                }
                super.onEaten(player, potionEffects);
