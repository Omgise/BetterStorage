...
import net.mcft.betterstorage.container.ContainerBetterStorage;
import net.mcft.betterstorage.container.SlotArmorBackpack;
import net.mcft.betterstorage.content.BetterStorageTiles;
import net.mcft.betterstorage.inventory.InventoryBackpackEquipped;
...
public class ItemBackpack extends ItemArmorBetterStorage implements ISpecialArmor, IDyeableItem {
    ...
    public static final ArmorMaterial material = EnumHelper
        .addArmorMaterial("backpack", 14, new int[] { 0, 2, 0, 0 }, 15);
    static {
        material.customCraftingMaterial = Items.leather;
    }

    protected ItemBackpack(ArmorMaterial material) {
        super(material, 0, 1);
    }
    ...
    protected IInventory getBackpackItemsInternal(EntityLivingBase carrier, EntityPlayer player) {
        ...
        if (backpackData.contents == null) backpackData.contents = new ItemStack[size];
    ...
    public ModelBackpack getModel() {
        if (model == null) {
            try {
                model = getModelClass().getConstructor(boolean.class)
                    .newInstance(true);
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public ModelBiped getArmorModel(EntityLivingBase entity, ItemStack stack, int slot) {
        if (modelArmor == null) {
            try {
                ModelBackpack model = getModelClass().getConstructor(boolean.class)
                    .newInstance(false);
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean advancedTooltips) {
        ...
        if (getBackpack(player) == stack) {
            ...
            } else if (enableHelpTooltips) LanguageUtils.translateTooltip(
                list,
            ...
            LanguageUtils.translateTooltip(list, (chestplate ? "backpack.equipHint" : "backpack.equipHint.extended"));
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        ForgeDirection orientation = DirectionUtils.getOrientation(player)
            .getOpposite();
    ...
    private static final String[] immuneToDamageType = { "inWall", "drown", "starve", "cactus", "fall", "outOfWorld",
    ...
    protected boolean takesDamage(ItemStack stack, DamageSource source) {
        ...
        for (String immune : immuneToDamageType) if (immune.equals(source.getDamageType())) return false;
        ...
        return (!enchantmentProtection(stack, Enchantment.protection, 0.3, 0.35, 0.4, 0.45)
            && !(source.isProjectile()
                && enchantmentProtection(stack, Enchantment.projectileProtection, 0.4, 0.5, 0.6, 0.7))
            && !(source.isFireDamage()
                && enchantmentProtection(stack, Enchantment.fireProtection, 0.55, 0.65, 0.75, 0.85))
            && !(source.isExplosion()
                && enchantmentProtection(stack, Enchantment.blastProtection, 0.65, 0.75, 0.85, 0.95)));
    }
    ...
    public static boolean onPlaceBackpack(EntityPlayer player, int x, int y, int z, int side) {
        ...
        if (!ItemBackpack.isBackpackOpen(player)) {
            ...
            success = backpack.getItem()
                .onItemUse(backpack, player, player.worldObj, x, y, z, side, 0, 0, 0);
        ...
        // Make sure the client has the same information as the server. It does not sync when backpackChestplate is
        // disabled because there are no changes to the slot in that case.
        if (!player.worldObj.isRemote && success
     ...
     */
    public static boolean placeBackpack(EntityLivingBase carrier, EntityPlayer player, ItemStack backpack, int x, int y,
        int z, int side, ForgeDirection orientation, boolean despawn, boolean deathDrop) {
        ...
        if ((player != null)
            && (!world.canMineBlock(player, x, y, z) || !player.canPlayerEdit(x, y, z, side, backpack))) return false;
        ...
        if (!world.setBlock(x, y, z, blockBackpack, orientation.ordinal(), 3)) return false;

        if (world.getBlock(x, y, z) != blockBackpack) return false;
        ...
        if (ItemBackpack.getBackpack(carrier) == backpack) te.unequip(carrier, despawn);
