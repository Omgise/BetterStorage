...
import net.mcft.betterstorage.tile.crate.CratePileData;
...
public class InventoryCratePlayerView extends InventoryBetterStorage implements ICrateWatcher {
    ...
    public InventoryCratePlayerView(TileEntityCrate crate) {
        ...
        Iterator<ItemStack> iter = data.getContents()
            .getRandomStacks()
            .iterator();
    ...
    public void setInventorySlotContents(int slot, ItemStack stack) {
        ...
        if (stack == null) onSlotEmptied(slot);
    }
    ...
    public ItemStack decrStackSize(int slot, int amount) {
        ...
        if (stack.stackSize <= 0) tempContents[slot] = null;

        ignoreModifiedItems = true;
        ItemStack result = data.removeItems(item, amount);
        ignoreModifiedItems = false;

        if (tempContents[slot] == null) onSlotEmptied(slot);
    ...
    private void onSlotEmptied(int slot) {
        int emptySlots = 0;
        for (int s = 0; s < tempContents.length; s++) if (tempContents[s] == null) emptySlots++;
    ...
    private int modifyItemsInSlot(int slot, ItemStack stack, MapData itemData, int amount) {
        ...
        if ((stack.stackSize += count) <= 0) tempContents[slot] = null;
