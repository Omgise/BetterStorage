...
public class InventoryTileEntity extends InventoryBetterStorage {
    ...
    private static ItemStack[][] getAllContents(TileEntityContainer... tileEntities) {
        ...
        for (int i = 0; i < tileEntities.length; i++) allContents[i] = tileEntities[i].contents;
        return allContents;
    }

    @Override
    public String getInventoryName() {
        return mainTileEntity.getContainerTitle();
    }
    ...
    public boolean isUseableByPlayer(EntityPlayer player) {
        for (TileEntityContainer tileEntity : tileEntities) if (!tileEntity.canPlayerUseContainer(player)) return false;
        return true;
    }
    ...
    public void markDirty() {
        inventory.markDirty();
        for (TileEntityContainer te : tileEntities) te.markDirtySuper();
    }
