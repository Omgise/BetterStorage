...
import net.mcft.betterstorage.network.packet.PacketBackpackStack;
import net.mcft.betterstorage.network.packet.PacketBackpackTeleport;
import net.mcft.betterstorage.network.packet.PacketDrinkingHelmetUse;
...
public class ChannelHandler extends SimpleNetworkWrapper {
    ...
    public <T extends IMessage & IMessageHandler<T, IMessage>> void register(int id, Side receivingSide,
    ...
    public void sendToAllAround(IMessage message, World world, double x, double y, double z, double distance,
        ...
        for (EntityPlayer player : (List<EntityPlayer>) world.playerEntities) {
            ...
            if ((dx * dx + dy * dy + dz * dz) < (distance * distance)) sendTo(message, player);
        }
    }
    ...
    public void sendToAllTracking(IMessage message, Entity entity) {
        ((WorldServer) entity.worldObj).getEntityTracker()
            .func_151247_a(entity, getPacketFrom(message));
    }
    ...
    public void sendToAndAllTracking(IMessage message, Entity entity) {
        ...
        if (entity instanceof EntityPlayer) sendTo(message, (EntityPlayer) entity);
    }
