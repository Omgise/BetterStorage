...
public class TileEntityPresentRenderer extends TileEntitySpecialRenderer {
    ...
    public void renderTileEntityAt(TileEntityPresent present, double x, double y, double z, float partialTicks) {
        ...
        TextureManager texMan = Minecraft.getMinecraft()
            .getTextureManager();
        ...
        if ((present.nameTag != null) && (present.getWorldObj() != null)
            && (player != null)
            && (player.getDistanceSq(present.xCoord + 0.5, present.yCoord + 0.5, present.zCoord + 0.5)
                < NAMETAG_RENDER_RANGE_SQ)) {
            MovingObjectPosition o = Minecraft.getMinecraft().objectMouseOver;
            renderNameTag(
                present.nameTag,
                player.getCommandSenderName()
                    .equalsIgnoreCase(present.nameTag),
