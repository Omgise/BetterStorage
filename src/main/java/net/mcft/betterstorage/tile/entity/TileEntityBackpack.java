...
public class TileEntityBackpack extends TileEntityContainer {
    ...
    public void unequip(EntityLivingBase carrier, boolean despawn) {
        ...
        if (backpackData.contents != null) {
            System.arraycopy(
                ...
                contents,
                0,
                Math.min(contents.length, backpackData.contents.length));
    ...
    public boolean onBlockBreak(EntityPlayer player) {
        ...
        if (!worldObj.isRemote && player.isSneaking() && ItemBackpack.canEquipBackpack(player)) equip(player);
    ...
    public void dropContents() {
        ...
        if (!brokenInCreative) WorldUtils.dropStackFromBlock(this, stack);
    ...
    public void updateEntity() {
        ...
        if ((lidAngle > 0.0F) && (prevLidAngle <= 0.0F)) worldObj.playSoundEffect(x, y, z, sound, 1.0F, 0.6F);
        // Play sound when closing
        if ((lidAngle < 0.2F) && (prevLidAngle >= 0.2F)) worldObj.playSoundEffect(x, y, z, sound, 0.8F, 0.4F);

        if (despawnTime < 0) return;
        if (despawnTime++ > 20 * 60 * 5) {
            equipped = true; // Prevents stuff from being dropped.
            worldObj.setBlockToAir(xCoord, yCoord, zCoord);
        } else if (((despawnTime % 40) == 0) && (worldObj.getClosestPlayer(x, y, z, 24) != null)) despawnTime = -1;
    }
