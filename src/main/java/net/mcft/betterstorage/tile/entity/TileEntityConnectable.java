...
public abstract class TileEntityConnectable extends TileEntityContainer implements IInventory {
    ...
    public TileEntityConnectable getMainTileEntity() {
        ...
        if (BetterStorage.globalConfig.getBoolean(GlobalConfig.enableWarningMessages)) BetterStorage.log.warn(
            "getConnectedTileEntity() returned null in getMainTileEntity(). " + "Location: {},{},{}",
            xCoord,
            yCoord,
            zCoord);
        return this;
    ...
    public void disconnect() {
        ...
        } else if (BetterStorage.globalConfig.getBoolean(GlobalConfig.enableWarningMessages)) BetterStorage.log.warn(
            "getConnectedTileEntity() returned null in disconnect(). " + "Location: {},{},{}",
            xCoord,
            yCoord,
            zCoord);
    }
    ...
    protected void onBlockPlacedBeforeCheckingConnections(EntityLivingBase player, ItemStack stack) {
        setOrientation(
            DirectionUtils.getOrientation(player)
                .getOpposite());
    }
    ...
    public void openInventory() {
        if (isAccessible()) getPlayerInventory().openInventory();
    }

    @Override
    public void closeInventory() {
        if (isAccessible()) getPlayerInventory().closeInventory();
    }

    @Override
    public void markDirty() {
        if (isAccessible()) getPlayerInventory().markDirty();
    }
