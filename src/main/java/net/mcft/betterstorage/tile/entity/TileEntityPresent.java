...
import net.mcft.betterstorage.network.packet.PacketPresentOpen;
...
public class TileEntityPresent extends TileEntityCardboardBox {
    ...
    protected void onItemDropped(ItemStack stack) {
        ...
        if (nameTag != null) compound.setString(TAG_NAMETAG, nameTag);
    ...
    public void updateEntity() {
        ...
        if (breakPause <= 0) breakProgress = Math.max(0, breakProgress - 1);
    }
    ...
    public boolean onBlockActivated(EntityPlayer player, int side, float hitX, float hitY, float hitZ) {
        ...
        if (holding == null) {
            ...
            if ((nameTag != null) && !player.getCommandSenderName()
                .equalsIgnoreCase(nameTag)) {
                breakPause = 40;
                if (!worldObj.isRemote) ((EntityPlayerMP) player).addChatMessage(
                    new ChatComponentText(EnumChatFormatting.YELLOW + "This present is for " + nameTag + ", not you!"));
                return false;
            }
            if ((breakProgress += 20) > 100) destroyed = true;
            ...
            worldObj
                .playSoundEffect(xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, sound, 1.0F, 0.4F + breakProgress / 160.0F);

            BetterStorage.networkChannel
                .sendToAllAround(new PacketPresentOpen(xCoord, yCoord, zCoord, destroyed), worldObj, x, y, z, 64);

            if (!destroyed) return true;

            if (BetterStorageTiles.cardboardBox != null) {
                if (worldObj.setBlock(xCoord, yCoord, zCoord, BetterStorageTiles.cardboardBox)) {
                    TileEntityCardboardBox box = WorldUtils
                        .get(worldObj, xCoord, yCoord, zCoord, TileEntityCardboardBox.class);
