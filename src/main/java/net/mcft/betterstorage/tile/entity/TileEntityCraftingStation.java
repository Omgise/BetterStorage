...
import net.mcft.betterstorage.container.ContainerCraftingStation;
...
public class TileEntityCraftingStation extends TileEntityContainer implements IInventory, ISidedInventory {
    ...
    public void dropContents() {
        for (ItemStack stack : crafting) WorldUtils.dropStackFromBlock(worldObj, xCoord, yCoord, zCoord, stack);
        if (stationInventory.outputIsReal)
            for (ItemStack stack : output) WorldUtils.dropStackFromBlock(worldObj, xCoord, yCoord, zCoord, stack);
        super.dropContents();
    }
    ...
    public boolean canExtractItem(int slot, ItemStack stack, int side) {
        return ((side != 0)
            || (GlobalConfig.enableStationAutoCraftingSetting.getValue() && stationInventory.canTake(null)));
    }
    ...
    public void readFromNBT(NBTTagCompound compound) {
        ...
        if (compound.hasKey("Output")) NbtUtils.readItems(output, compound.getTagList("Output", NBT.TAG_COMPOUND));
    ...
    public void writeToNBT(NBTTagCompound compound) {
        ...
        if (stationInventory.outputIsReal) compound.setTag("Output", NbtUtils.writeItems(output));
