...
public class CratePileMap {
    ...
    private boolean checkCuboid(int minX, int minY, int minZ, int maxX, int maxY, int maxZ) {
        for (int x = minX; x <= maxX; x++)
            for (int y = minY; y <= maxY; y++) for (int z = minZ; z <= maxZ; z++) if (get(x, y, z)) return false;
        return true;
    }
        ...
        if (region.contains(x, y, z)) {
            ...
            if (!value) {
                ...
                if (region.minX - mapRegion.minX > 4 || mapRegion.maxX - region.maxX > 4
                    || region.minY / 8 - mapRegion.minY > 2
                    || mapRegion.maxY / 8 - region.maxY > 2
                    || region.minZ - mapRegion.minZ > 4
                    || mapRegion.maxZ - region.maxZ > 4) resize();
            }
        } else if (value) {
            region.expandToContain(x, y, z);
            if (!mapRegion.contains(x, y / 8, z)) resize();
            internalSet(x - mapRegion.minX, y - mapRegion.minY * 8, z - mapRegion.minZ, value);
        }
    }
