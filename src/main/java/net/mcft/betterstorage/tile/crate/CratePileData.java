...
import net.mcft.betterstorage.inventory.InventoryCrateBlockView;
import net.mcft.betterstorage.misc.ItemIdentifier;
import net.mcft.betterstorage.misc.Region;
...
public class CratePileData {
    ...
    public void markDirty() {
        ...
        dirty = true;
        if (BetterStorage.globalConfig.getBoolean(GlobalConfig.crateDebugMessages)) BetterStorage.log.info(
            String.format(
                "[CRATE DEBUG] Marked crate pile at [%s,%s,%s] as dirty.",
                getCenterX(),
                getCenterY(),
                getCenterZ()));
    }
    ...
    public void save() {
        ...
        dirty = false;
        if (BetterStorage.globalConfig.getBoolean(GlobalConfig.crateDebugMessages)) BetterStorage.log.info(
            String.format("[CRATE DEBUG] Saved crate pile at [%s,%s,%s].", getCenterX(), getCenterY(), getCenterZ()));
    }
    ...
    public void remove() {
        ...
        if (BetterStorage.globalConfig.getBoolean(GlobalConfig.crateDebugMessages)) BetterStorage.log.info(
            String.format(
                "[CRATE DEBUG] Removed empty crate pile at [%s,%s,%s].",
                getCenterX(),
                getCenterY(),
                getCenterZ()));
    }
    ...
    private boolean canExpand(TileEntityCrate crate) {
        ...
        if (crate.xCoord < map.region.minX || crate.xCoord > map.region.maxX) {
            ...
            if (map.region.width() >= maxDiff + Math.min(map.region.height(), map.region.depth())) return false;
        } else if (crate.zCoord < map.region.minZ || crate.zCoord > map.region.maxZ) {
            int maxDiff = ((map.region.width() == 1) ? 1 : 3);
            if (map.region.height() >= maxDiff + Math.min(map.region.width(), map.region.depth())) return false;
        } else if (crate.yCoord < map.region.minY || crate.yCoord > map.region.maxY) {
            int maxDiff = ((map.region.width() == 1 || map.region.height() == 1) ? 1 : 4);
            if (map.region.depth() >= maxDiff + Math.min(map.region.width(), map.region.height())) return false;
        }
        return true;
    }
    ...
    public ItemStack addItems(ItemStack stack) {
        ...
        if (space > 0) {
            ...
            if (space < stack.stackSize) overflow = stack.splitStack(stack.stackSize - space);
    ...
    public int getSpaceForItem(ItemIdentifier item) {
        ...
        if (amount > 0) space += (StackUtils.calcNumStacks(testStack) * maxStackSize) - testStack.stackSize;
        return space;
    }
    ...
    public NBTTagCompound toCompound() {
        ...
        for (ItemStack stack : getContents().getItems()) {
            ...
            if (stack.hasTagCompound()) stackCompound.setTag("tag", stack.getTagCompound());
            stacks.appendTag(stackCompound);
        }
        compound.setTag("stacks", stacks);
        if (map != null) compound.setTag("map", map.toCompound());
        return compound;
    }
    ...
    public static CratePileData fromCompound(CratePileCollection collection, int crateId, NBTTagCompound compound) {
        ...
        for (int j = 0; j < stacks.tagCount(); j++) {
            ...
            if (stackCompound.hasKey("tag")) stack.stackTagCompound = stackCompound.getCompoundTag("tag");
            if (stack.getItem() != null) pileData.getContents()
                .set(new ItemIdentifier(stack), stack.stackSize);
        }
        if (compound.hasKey("map")) pileData.map = CratePileMap.fromCompound(compound.getCompoundTag("map"));
        return pileData;
    }
