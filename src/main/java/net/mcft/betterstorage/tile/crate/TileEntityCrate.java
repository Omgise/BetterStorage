...
import net.mcft.betterstorage.api.crate.ICrateStorage;
import net.mcft.betterstorage.api.crate.ICrateWatcher;
...
import net.mcft.betterstorage.inventory.InventoryCratePlayerView;
import net.mcft.betterstorage.misc.Constants;
import net.mcft.betterstorage.misc.ItemIdentifier;
import net.mcft.betterstorage.tile.entity.TileEntityContainer;
...
public class TileEntityCrate extends TileEntityContainer implements IInventory, ICrateStorage, ICrateWatcher {
    ...
    private static final ForgeDirection[] sideDirections = { ForgeDirection.NORTH, ForgeDirection.SOUTH,
    ...
    public CratePileData getPileData() {
        if (worldObj.isRemote) throw new IllegalStateException("Can't be called client-side.");
        if (data == null) {
            CratePileCollection collection = CratePileCollection.getCollection(worldObj);
            if (id == -1) setPileData(collection.createCratePile(), true);
    ...
    private void setPileData(CratePileData data, boolean addCrate) {
        if (this.data != null) this.data.removeCrate(this);
        this.data = data;
        if (data != null) {
            id = data.id;
            markForUpdate();
    ...
    private void checkPileConnections(CratePileData data) {
        ...
        for (int i = 1; i < crateSets.size(); i++) {
            ...
            for (TileEntityCrate newPileCrate : set) {
                ...
                while (true) {
                    newPileCrate = WorldUtils.get(
                        worldObj,
                        newPileCrate.xCoord,
                        newPileCrate.yCoord + 1,
                        newPileCrate.zCoord,
            ...
            for (ItemStack stack : data.getContents()
                .getRandomStacks(count)) {
    ...
    private List<HashSet<TileEntityCrate>> getCrateSets(int x, int y, int z, CratePileData data) {
        ...
        for (ForgeDirection dir : sideDirections) {
            ...
            for (HashSet<TileEntityCrate> set : crateSets) if (set.contains(neighborCrate)) continue neighborLoop;
    ...
    private void checkConnections(int x, int y, int z, CratePileData data, HashSet<TileEntityCrate> set) {
        ...
        set.add(crate);
        for (ForgeDirection ndir : sideDirections) checkConnections(x + ndir.offsetX, y, z + ndir.offsetZ, data, set);
    }
    ...
    public void attemptConnect(ForgeDirection side) {
        ...
        if (pileData.canAdd(this)) setPileData(pileData, true);
    }
    ...
    public void invalidate() {
        ...
        if (watcherRegistered) data.removeWatcher(this);
    ...
    private void dropItems(List<ItemStack> stacks) {
        for (ItemStack stack : stacks) dropItem(stack);
    }
    ...
    private void dropOverflowContents(CratePileData data) {
        ...
        if (amount <= 0) return;
        List<ItemStack> items = data.getContents()
            .getRandomStacks(amount);
    ...
    public void openGui(EntityPlayer player) {
        ...
        PlayerUtils.openGui(
            ...
            getName(),
            getColumns(),
            2 * Math.min(data.getNumCrates(), 3),
            getContainerTitle(),
    ...
    public void markDirty() {
        if (GlobalConfig.enableCrateInventoryInterfaceSetting.getValue()) getPileData().blockView.markDirty();
    }
    ...
    public Iterable<ItemStack> getContents() {
        return (isEnabled() ? getPileData().getContents()
            .getItems() : Collections.EMPTY_LIST);
    }

    @Override
    public Iterable<ItemStack> getRandomStacks() {
        return (isEnabled() ? getPileData().getContents()
            .getRandomStacks() : Collections.EMPTY_LIST);
    }

    @Override
    public int getItemCount(ItemStack identifier) {
        return (isEnabled() ? getPileData().getContents()
            .get(new ItemIdentifier(identifier)) : 0);
    }
    ...
    public void registerCrateWatcher(ICrateWatcher watcher) {
        if (isEnabled()) getPileData().addWatcher(watcher);
    }

    @Override
    public void unregisterCrateWatcher(ICrateWatcher watcher) {
        if (isEnabled()) getPileData().removeWatcher(watcher);
    }
