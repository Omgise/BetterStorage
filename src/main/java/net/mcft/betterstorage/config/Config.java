...
import net.mcft.betterstorage.config.setting.DoubleSetting;
import net.mcft.betterstorage.config.setting.IntegerSetting;
import net.mcft.betterstorage.config.setting.Setting;
...
public class Config {
    ...
    public int getInteger(String name) {
        return ((IntegerSetting) get(name)).getValue();
    }

    /** Returns the value of a double setting with that name. */
    public double getDouble(String name) {
        return ((DoubleSetting) get(name)).getValue();
    }

    /** Returns the value of a boolean setting with that name. */
    public boolean getBoolean(String name) {
        return ((BooleanSetting) get(name)).getValue();
    }

    /** Loads settings from the config file. */
    public void load() {
        forgeConfig.load();
        for (Setting setting : settings.values()) setting.load(forgeConfig);
    }

    /** Saves settings to the config file. */
    public void save() {
        for (Setting setting : settings.values()) setting.save(forgeConfig);
        forgeConfig.save();
    }
    ...
    public void validate() {
        for (Setting setting : settings.values()) setting.validate();
    }

    /** Synchronizes all settings with those of the server. */
    public void read(NBTTagCompound compound) {
        for (Setting setting : syncedSettings) setting.read(compound);
    }
    ...
    public void write(NBTTagCompound compound) {
        for (Setting setting : syncedSettings) setting.write(compound);
    }
